...
1. **Fix the local-date generation bug first (correctness, spec v3 §2.2).**
   - `frontend/src/components/HabitTable.tsx:34` and `frontend/src/components/CalendarView.tsx:18` still build day keys with `new Date(y, m - 1, d).toISOString().slice(0, 10)`; replace this with shared local `YYYY-MM-DD` formatting that never round-trips through UTC.
   - Prefer one shared helper (for example `frontend/src/lib/date.ts`) so the bug is fixed once instead of in two copy-pasted `daysInMonth()` implementations.
   - Update tests/fixtures that also derive local day keys with `toISOString()` (notably `frontend/src/components/__tests__/HabitApp.test.tsx`) so the bug is actually asserted, not masked.

2. **Build the v3 To-Do feature next (net-new feature, spec v3 §3).**
...
3. **Finish the v3 habit interaction work (spec v3 §2.3).**
...
4. **Apply the v3 UI modernization pass (spec v3 §2.1).**
...
5. **Close the highest-value carried-forward habit-spec gaps that v3 does not restate in detail.**
...
6. **Keep docs and validation ergonomics aligned as features land.**
