...
- **Migrations troubleshooting**: `PYTHONPATH=src flask --app src.app db heads` should show one head; use `PYTHONPATH=src flask --app src.app db upgrade` to verify/apply migrations directly if `script/setup` fails (PYTHONPATH is required for these `flask` CLI commands, same as pytest)
